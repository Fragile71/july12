 ...
 *        이런한 기술을 IoC 컨테이너가 관리, 서비스 해줍니다.
 *        IOC컨테이너는 객체의 생성/초기화/서비스/소멸 --> (라이프사이클을 관리해줍니다.)
 *        
 * 3. AOP(Aspect Oriented programming) 관점지향 프로그래밍
 *     객체지향과 다른 개념. 각 객체마다 중복되는 작업들이 있다면 해당
 *     중복 작업을 기준으로 동일 행동을 하게 만드는 것.
 *     메서드가 실행되기전, 실행된 직후, 실행시점에 따라 따로 기능을 
 */
...
//servlet에서는 클래스하나가 제어 여기선 메소드하나가 제어

	@RequestMapping(value = "/main")
	public String main() {
		return "name";
		// 위 문장과 연결되어서 /main이라는 요청이오면
		// jsp파일명 아래 해당 jsp파일을 열어줍니다.
